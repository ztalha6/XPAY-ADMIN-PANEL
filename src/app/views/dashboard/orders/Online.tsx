...
export default function Online() {
    ...
    const fetchData = async (params: IDatatableParams = {},data:any) => {
        setLoading(true);
        data.type = BACKEND_CONSTANTS.ORDERS.TYPES.ONLINE
    ...
    const columns: ColumnsType<IOrderList> = [
        ...
        {
            title: 'Customer Name',
            dataIndex: 'mobile_customer',
            width: '280px',
            render: (mobile_customer)=> <SkeletonTableCell loading={loading} value={mobile_customer?.full_name}/>,
            sorter: (a, b) => a?.mobile_customer ? a?.mobile_customer?.full_name?.localeCompare(b?.mobile_customer?.full_name || ""):0,
            sortDirections: ['descend','ascend'],
        },
        {
            title: 'Created Date',
